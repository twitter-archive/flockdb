...
object SelectQuery {
  class RichFlockSelectQuery(query: flockdb.SelectQuery) {
    def toThrift = {
      val out = new thrift.SelectQuery(query.operations.map { _.toThrift }.toJavaList,
                                          query.page.toThrift)
      out.setUser_timeout_ms(query.userTimeoutMS)
      out
    }
  }
  ...
  class RichThriftSelectQuery(query: thrift.SelectQuery) {
    def fromThrift = new flockdb.SelectQuery(query.operations.toSeq.map { _.fromThrift }.toList,
                                           query.page.fromThrift, query.user_timeout_ms)
  }
