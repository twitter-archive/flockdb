...
object UnsafeAddQuery {
  class RichFlockUnsafeAddQuery(term: flockdb.UnsafeAddQuery) {
    def toThrift = new thrift.UnsafeAddQuery(term.sourceId, term.graphId, term.destinationId, term.createdAt.inSeconds)
  }
  ...
  class RichThriftUnsafeAddQuery(term: thrift.UnsafeAddQuery) {
    def fromThrift = new flockdb.UnsafeAddQuery(term.source_id, term.graph_id, term.destination_id, Time(term.created_at.seconds))
  }
