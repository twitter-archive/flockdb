...
class ReadWriteShardAdapter(shard: shards.ReadWriteShard[Shard])
  ...
  def addUnsafe(sourceId: Long, destinationId: Long, position: Long, updatedAt: Time)                      = shard.writeOperation(_.addUnsafe(sourceId, destinationId, position, updatedAt))
