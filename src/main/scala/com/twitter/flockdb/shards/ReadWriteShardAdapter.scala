  ...
  def withLock[A](sourceId: Long)(f: (Shard, Metadata) => A) = {
    if (shard.isInstanceOf[shards.ReplicatingShard[_]]) {
      ...
      lockServer.withLock(sourceId) { (lock, metadata) =>
        f(new ReadWriteShardAdapter(new shards.ReplicatingShard(shardInfo, weight, List(lock) ++ rest, replicatingShard.loadBalancer, replicatingShard.future, 6.seconds)), metadata)
      }
    } else {
