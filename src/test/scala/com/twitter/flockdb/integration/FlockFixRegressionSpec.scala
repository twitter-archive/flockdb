...
class FlockFixRegressionSpec extends IntegrationSpecification {
  ...
  "select results" should {
    "be in order and still in order after unarchive" in {
      ...
      val job = new Multi(alice, FOLLOWS, Direction.Forward, State.Normal, Time.now, flockdb.Priority.High, pageSize, flock.forwardingManager, flock.jobScheduler, NoOpFilter)
      job()
